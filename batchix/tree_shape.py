...
def pytree_get_shape_first_n_equal(x: PyTree, first_n_shape_elements=1, custom_error_msg: str = None) -> Tuple[int, ...]:
    """
    ...
    def reduce_x_shape_zero(pre_shape, el):
        assert len(el.shape) >= first_n_shape_elements, \
...
def pytree_get_shape_last_n_equal(x: PyTree, last_n_shape_elements=1) -> Tuple[int, ...]:
    """
