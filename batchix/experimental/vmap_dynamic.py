...
class JaxVMapFlexibleLength:
    ...
    def __call__(self, x: X) -> Y:
        ...
        x_shape_zero: int = jax.tree_util.tree_reduce(
            reduce_x_shape_zero, x, initializer=None
        )
        ...
        x = jax.tree_util.tree_map(
            lambda el: jnp.concat(
                [el, jnp.repeat(el[-2:-1], axis=0, repeats=x_remain_elements)], axis=0
            ),
            x,
        )
        ...
        for i in range(num_iterations):
            x_sub = jax.tree_util.tree_map(
                lambda el: el[i * self.vmap_batch_size : (i + 1) * self.vmap_batch_size]
                if self.vmap_batch_size > 1
                else el[i * self.vmap_batch_size],
                x,
            )
            # test
            ...
            if result_all is None:
                ...
                result_all = jax.tree_util.tree_map(
                    lambda el: jnp.zeros(
                        (num_iterations * self.vmap_batch_size,)
                        + el.shape[1 if self.vmap_batch_size > 1 else 0 :]
                    ),
                    result,
                )
            # set result
            ...
            result_all = jax.tree_util.tree_map(
                lambda el, new_el: (
                    el.at[i * self.vmap_batch_size : (i + 1) * self.vmap_batch_size]
                    if self.vmap_batch_size > 1
                    else el.at[i * self.vmap_batch_size]
                ).set(new_el),
                result_all,
                result,
            )
